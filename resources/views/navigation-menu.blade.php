<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @if(auth()->user()->can('view-activity-log') || auth()->user()->hasRole('Super User'))
                        <x-jet-nav-link href="{{ route('log.management') }}" :active="request()->routeIs('log.management')">
                            {{ __('Activity Logs') }}
                        </x-jet-nav-link>
                    @endif
                </div>
                ...
                <div class="ml-3 relative">
                    <x-jet-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                    ...
                                    <span style="padding-top: 7px;">&nbsp;{{ Auth::user()->full_name }}</span>
                                </button>
                            @else
                                <span class="inline-flex rounded-md">
                                    <button type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition">
                        ...
                        <x-slot name="content">
                            <!-- Account Management -->
                            <div class="block px-4 py-2 text-xs text-gray-400">
                                @php $role = \Illuminate\Support\Facades\Auth::user()->getRoleNames()->first(); @endphp
                                {{ __("Manage $role Account") }}
                            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-jet-responsive-nav-link href="{{ route('dosen') }}" :active="request()->routeIs('dosen')">
                {{ __('Dosen') }}
            </x-jet-responsive-nav-link>
